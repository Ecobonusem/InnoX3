...
def send_to_admin(context: CallbackContext, chat_id): data = user_data[chat_id] text = ( f"🎂 Yangi ro‘yxatdan o‘tuvchi:\n" f"📍 Viloyat: {data['region']}\n" f"🏘 Tuman: {data['district']}\n" f"👤 F.I.Sh.: {data['fish']}\n" f"📞 Tel: {data['phone']}\n" f"📝 Loyiha: {data['project']}" ) context.bot.send_message(chat_id=ADMIN_ID, text=text) if data.get("photo"): context.bot.send_photo(chat_id=ADMIN_ID, photo=data["photo"])
